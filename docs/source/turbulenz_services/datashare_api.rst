...
.. _datashare_locking_resources:

**Locking resources**

Resources can be locked by using the :ref:`compareAndSet <datashare_compareandset>` function.
Suppose that, for a strategy game, only one player can attack another players base 'player-base' key at a time.
Before the player can attack they write to a 'player-base-lock' key::

    function acquireLock(store)
    {
        var lockState = JSON.parse(store.value);
        if (store.lastSet + lockState.time >= TurbulenzServices.getServerTime())
        {
            // unable to acquire the lock
            return;
        }
        function compareAndSetCallback(wasSet, reason)
        {
            if (wasSet)
            {
                // player has the lock
        ...
        // attempt to lock the player-base key for 60 seconds
        dataShare.compareAndSet({
                key: 'player-base-lock',
                value: JSON.stringify({
                    time: 60
                }),
                callback: compareAndSetCallback
            });
    }

    dataShare.get('player-base-lock', acquireLock);

Note that we only lock the key for a set period.
This way the game can recover if a user loses their connection while holding onto the resource.

**Tic-tac-toe**

Here is a tic-tac-toe example::

    var gameState = {}; // all of the current games being played
    var dataShareManager = DataShareManager.create(requestHandler, gameSession);

    // create a data share and set up any initial game state
    function hostCreateGame()
    {
        ...
        function dataShareCreated(dataShare) {
            gameState[dataShare.id] = newGameState;
            function compareAndSetCallback(wasSet, reason)
            {
                if (wasSet)
                {
            ...
            dataShare.compareAndSet({
                    key: 'gamestate',
                    value: JSON.stringify(newGameState),
                    callback: compareAndSetCallback
                });
        }
        dataShareManager.createDataShare(dataShareCreated);
    ...
    // join another players game (if they have created one)
    function clientJoinGame(username)
    {
        function getJoinedFn(dataShare)
        {
            return function joined() {
                    Utilities.log('Joined game');
                    dataShare.get('gamestate', function stateLoaded(store)
                        {
                            var newGameState = JSON.parse(store.value);
                            gameState[dataShare.id] = newGameState;
                        });
                };
        }
        ...
        dataShareManager.findDataShares({
            ...
            callback: function callback(dataShares)
            {
                ...
                {
                    dataShare = dataShares[dataSharesIndex];
                    if (dataShare.users < 2)
                    {
                        dataShare.join(getJoinedFn(dataShare));
                        return;
                    }
                }
                Utilities.log('User not in a lobby');
            }
        });
    }

    // read other players moves and update the game state
    function readShare()
    {
        function getGameState(dataShare)
        {
            dataShare.get('gamestate', function stateLoaded(store) {
                    var newGameState = JSON.parse(store.value);
                    gameState[dataShare.id] = newGameState;
        ...
        dataShareManager.findDataShares({
            callback: function callback(dataShares)
            {
                ...
                {
                    dataShare = dataShares[dataSharesIndex];
                    getGameState(dataShare);
                }
            }
        });
    }

    // make a move and send it to the other players
    function onPlayerMove(dataShare, x, y)
    {
        var currentGameState = gameState[dataShare.id];
        ...
        function compareAndSetCallback(wasSet, reason) {
            ...
            {
                // the value has changed since the last get
                dataShare.get('gamestate', dataShareGetCallback);
            }
        }

        if (!squareTaken(x, y))
        {
            currentGameState.moves.append([x, y]);

            dataShare.compareAndSet({
                    key: 'gamestate',
                    value: JSON.stringify(currentGameState),
                    callback: compareAndSetCallback
                });
        }
    }

    // the game is over or the player forfeits the game
    function onPlayerLeave(dataShare)
    {
        dataShare.leave(function callback()
        {
            // player now stops playing the game
        });
    }
...
For an example of how to use ``DataShare.compareAndSet`` see :ref:`locking resources <datashare_locking_resources>`.
