...
Turbulenz SDK 0.21.3 is an update for both the Turbulenz Engine
...
.. NOTE::
    ...
    If you intended to develop using this SDK, you will need to install the engine located here: **\*SDKINSTALLDIR\*/engines/0.21.3/**.
...
**Windows XP/Vista/7**
    ...
    * PDF reader - Required to read documentation in PDF format
    ...
    2) Download SDK 0.21.3 for Windows
    3) Run TurbulenzSDK-0.21.3.exe
...
**Mac OS X 10.6/10.7 (BETA)**
    ...
    * Cg Toolkit - http://developer.nvidia.com/cg-toolkit - Required
    ...
    1) Visit https://hub.turbulenz.com and login
    2) Download SDK 0.21.3 for Mac
    3) Open a terminal in the same directory as the SDK installer and type:
    4) chmod a+x TurbulenzSDK-0.21.3.run
    5) ./TurbulenzSDK-0.21.3.run
...
**Linux (ALPHA)**
    ...
    * Cg Toolkit 3.0+
    ...
    1) Visit https://hub.turbulenz.com and login
    2) Download SDK 0.21.3 for Linux
    3) Open a terminal in the same directory as the SDK installer and type:
    4) chmod a+x TurbulenzSDK-0.21.3.linux.run
    5) ./TurbulenzSDK-0.21.3.linux.run
    6) Follow the instructions
...
- Alternative(s)
    :Windows: Firefox 15 with Firebug 1.10.3
    :Mac: Safari 5.1 with Web Inspector (built-in)

For plugin configurations: **Firefox with Firebug**

- Firefox 15 is recommended in debug mode. For good performance it
...
* Simpified and optimized the leaderboards scrolling functions.
  See :ref:`LeaderboardResult.pageUp <leaderboardresult_pageup>`
  , :ref:`LeaderboardResult.pageDown <leaderboardresult_pagedown>`
  , :ref:`LeaderboardResult.scrollUp <leaderboardresult_scrollup>`
  and :ref:`LeaderboardResult.scrollDown <leaderboardresult_scrolldown>`.
...
* Custom events now allow passing in an array of numbers as the event value.
  See :ref:`sendCustomMetricEvent <turbulenzservices_sendcustommetricevent>`.

* The :ref:`leaderboards.yaml <leaderboards_yaml>` now takes an optional set of default scores for the leaderboards.

* Added a :ref:`GameProfileManager <gameprofilemanager>` for sharing a small public game specific profile for each user.
...
* The :ref:`leaderboardManager.get <leaderboardmanager_get>` API has changed.
  For the same behavior as previous SDK's you must now call :ref:`getView() <leaderboardresult_getview>` on the
  returned result of the :ref:`leaderboardManager.get <leaderboardmanager_get>` request::

    function callbackFn(key, leaderboardResult)
    {
        var data = leaderboardResult.getView();
        var ranking = data.ranking;
        ...
    }
    leaderboardManager.get(key, spec, callbackFn);

  The :ref:`leaderboardManager.get <leaderboardmanager_get>` now allows ``size`` to be up to 32 for top and near ``type`` requests.
...
* Fixed issue where the plugin version on mac could fail to connect to
