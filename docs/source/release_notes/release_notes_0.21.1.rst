...
Turbulenz SDK 0.21.1 is an update for both the Turbulenz Engine
...
.. NOTE::
    ...
    If you intended to develop using this SDK, you will need to install the engine located here: **\*SDKINSTALLDIR\*/engines/0.21.1/**.
...
**Windows XP/Vista/7**
    ...
    * PDF reader - Required to read documentation in PDF format
    ...
    2) Download SDK 0.21.1 for Windows
    3) Run TurbulenzSDK-0.21.1.exe
...
**Mac OS X 10.6/10.7 (BETA)**
    ...
    * Cg Toolkit - http://developer.nvidia.com/cg-toolkit - Required
    ...
    1) Visit https://hub.turbulenz.com and login
    2) Download SDK 0.21.1 for Mac
    3) Open a terminal in the same directory as the SDK installer and type:
    4) chmod a+x TurbulenzSDK-0.21.1.run
    5) ./TurbulenzSDK-0.21.1.run
...
**Linux (ALPHA)**
    ...
    * Cg Toolkit 3.0+
    ...
    1) Visit https://hub.turbulenz.com and login
    2) Download SDK 0.21.1 for Linux
    3) Open a terminal in the same directory as the SDK installer and type:
    4) chmod a+x TurbulenzSDK-0.21.1.linux.run
    5) ./TurbulenzSDK-0.21.1.linux.run
    6) Follow the instructions
...
* The :ref:`leaderboardManager.get <leaderboardmanager_get>` function now returns a :ref:`LeaderboardResult <leaderboardresult>` object that
  has 2 functions for paging up/down through the leaderboard scores ``leaderboardResult.getPageAbove``
  and ``leaderboardResult.getPageBelow``.
